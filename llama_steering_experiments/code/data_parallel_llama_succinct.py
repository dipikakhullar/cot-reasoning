...
STEP_BY_STEP_SUFFIX = "\nPlease make your response as succinct as possible, and put your final answer within \\boxed{}.\n"
...
def run_on_gpu(gpu_id, model_name, steering_vector_path, df, output_path, layer_idx):
    ...
    for idx, row in df.iterrows():
        ...
        if len(batch) >= batch_size:
            ...
            for i in range(len(batch)):
                results.append({
                    "uuid": uuids[i],
                    "input_text": texts[i],
                    "succinct_llama_16k": baseline_outputs[i],
                })
        ...
        if idx % 25 == 0:
            ...
            df_temp.to_parquet(f"{output_path}/gpu{gpu_id}_succinct_llama_16k_{idx}.parquet", index=False)
            print("done", f"{output_path}/gpu{gpu_id}_succinct_llama_16k_{idx}.parquet",  flush = True)
...
def launch_all_processes():
    ...
    output_path = "succinct_llama_16k"
