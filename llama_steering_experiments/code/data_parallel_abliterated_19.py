        ...
        if len(batch) >= batch_size:
            ...
            for i in range(len(batch)):
                results.append({
                    "uuid": uuids[i],
                    "input_text": texts[i],
                    "abl_19_output": baseline_outputs[i],
                })
            # Reset batch
            batch = []
            ...
            if idx % 50 == 0:
                ...
                df_temp.to_parquet(f"{output_path}/gpu{gpu_id}_abl_19_{idx}.parquet", index=False)
                print("done", f"{output_path}/gpu{gpu_id}_abl_19_{idx}.parquet",  flush = True)



def launch_all_processes():
    model_name = "dipikakhullar/DeepSeek-R1-Distill-Llama-8B-orthogonalized-19"
