...
def run_on_gpu(gpu_id, model_name, df, output_path):
    print("ID: ", gpu_id)
    ...
    for idx, row in df.iterrows():
        ...
        if len(batch) >= batch_size:
            ...
            baseline_outputs = generate_text_batch(model, tokenizer, texts, steering_vector=None, max_length=8000)
            ...
            for i in range(len(batch)):
                results.append({
                    "uuid": uuids[i],
                    "input_text": texts[i],
                    "abl_10_output": baseline_outputs[i],
                })
            # Reset batch
            batch = []
            ...
            if idx % 50 == 0:
                ...
                df_temp.to_parquet(f"{output_path}/gpu{gpu_id}_abl_10_{idx}.parquet", index=False)
                print("done", f"{output_path}/gpu{gpu_id}_abl_10_{idx}.parquet",  flush = True)



def launch_all_processes():
    model_name = "dipikakhullar/DeepSeek-R1-Distill-Llama-8B-orthogonalized-10"
    ...
    for i in range(num_gpus):
        ...
        p = mp.Process(target=run_on_gpu, args=(i, model_name, df_chunk, output_path))
        p.start()
        processes.append(p)
