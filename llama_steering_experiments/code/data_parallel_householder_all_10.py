        ...
        if len(batch) >= batch_size:
            ...
            for i in range(len(batch)):
                results.append({
                    "uuid": uuids[i],
                    "input_text": texts[i],
                    "householder_all_10_8k": baseline_outputs[i],
                })
            # Reset batch
            batch = []
            ...
            if idx % 5 == 0:
                ...
                df_temp.to_parquet(f"{output_path}/gpu{gpu_id}_house_all_10_8k_{idx}.parquet", index=False)
                print("done", f"{output_path}/gpu{gpu_id}_house_all_10_8k_{idx}.parquet",  flush = True)



def launch_all_processes():
    model_name = "dipikakhullar/DeepSeek-R1-Distill-Llama-8B-householder-all-10"
    df_path = "openr1_math_with_cot_cues.pkl"
    steering_vector_path = "steering_vectors_Wait.pt"
    layer_idx = 19
    num_gpus = torch.cuda.device_count()
    output_path = "householder_all_10_8k"
