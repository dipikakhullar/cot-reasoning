...
def run_on_gpu(gpu_id, model_name, steering_vector_path, df, output_path, layer_idx):
    ...
    # Load steering vector for layer 19
    steering_vectors = torch.load(steering_vector_path)
    layer_idx = 19
    steering_vector_19 = steering_vectors[layer_idx]
    ...
    for idx, row in df.iterrows():
        ...
        if len(batch) >= batch_size:
            ...
            print("steered_outputs batched")
            steered_outputs = generate_text_batch(model, tokenizer, texts, steering_vector=steering_vector_19, layer_index=layer_idx,  scale=.8, max_length=8000)
            print("anti_outputs batched")
            anti_outputs = generate_text_batch(model, tokenizer, texts, steering_vector=-steering_vector_19, layer_index=layer_idx,  scale=.8, max_length=8000)

            # Global anti-steering requires one-by-one (still slow)
            global_anti_outputs = []
            for text in texts:
                hook_handles = []
                for l in steering_vectors:
                    ...
                    # layer = model.model.layers[l]
                    set_steering_params(-steering_vectors[l])
                    hook = layer.register_forward_hook(simple_steering_hook)
                    hook_handles.append(hook)

                out = generate_text(model, tokenizer, text, steering_vector=None, layer_index=layer_idx,  scale=.8, max_length=8000)
                global_anti_outputs.append(out)
            ...
            for i in range(len(batch)):
                results.append({
                    "uuid": uuids[i],
                    "input_text": texts[i],
                    "steered_.8": steered_outputs[i],
                    "anti_steered_.8": anti_outputs[i],
                    "global_anti_steered_.8": global_anti_outputs[i]
                })
                # Reset batch
                batch = []
            ...
            if idx % 50 == 0:
                ...
                df_temp.to_parquet(f"{output_path}/gpu{gpu_id}_steering_output_{idx}.parquet", index=False)
                print("done", f"{output_path}/gpu{gpu_id}_steering_output_{idx}.parquet",  flush = True)
...
def launch_all_processes():
    ...
    output_path = "steering_outputs_parallel_.8"
    ...
    df = df.iloc[0:2000]  # <-- Only keep rows from index 1000 to 200
