        ...
        if len(batch) >= batch_size:
            ...
            for i in range(len(batch)):
                results.append({
                    "uuid": uuids[i],
                    "input_text": texts[i],
                    "abliterated_all_i": baseline_outputs[i],
                })
            # Reset batch
            batch = []
            ...
            if idx % 50 == 0:
                ...
                df_temp.to_parquet(f"{output_path}/gpu{gpu_id}_abl_all_i_{idx}.parquet", index=False)
                print("done", f"{output_path}/gpu{gpu_id}_abl_all_i_{idx}.parquet",  flush = True)



def launch_all_processes():
    model_name = "dipikakhullar/DeepSeek-R1-Distill-Llama-8B-orthogonalized-all-i"
    df_path = "openr1_math_with_cot_cues.pkl"
    steering_vector_path = "steering_vectors_Wait.pt"
    layer_idx = 19
    num_gpus = torch.cuda.device_count()
    output_path = "abl_all_i"
